# PLH South Sudan - Exploratory analysis
# Campbell McDuling
library(readxl)
library(psych)
...
#================================DESCRIPTIVE STATS 2=============================================
#read in matched pairs dataset
dat.join <- read.csv("dat.join.csv")
dat.join <- dat.join[,-1]

#matched pairs demographic information
#for caregivers
hist(dat.join$Dem_AgeCG.x, main="", xlab="Caregiver Age")
describe(dat.join[1:17], skew = F)
summary(as.factor(dat.join$Dem_Relationship))
(58+19)/85 #biological parent
1- (58+19)/85 #other

#father lives in HH
summary(as.factor(dat.join$Dem_Father.x.x))[1]/sum(summary(as.factor(dat.join$Dem_Father.x.x)))
#father deceased
summary(as.factor(dat.join$Dem_Father.x.x))[2]/sum(summary(as.factor(dat.join$Dem_Father.x.x)))
#mother lives in HH
summary(as.factor(dat.join$Dem_Mother.x.x))[1]/sum(summary(as.factor(dat.join$Dem_Mother.x.x)))
#mother deceased
summary(as.factor(dat.join$Dem_Mother.x.x))[2]/sum(summary(as.factor(dat.join$Dem_Mother.x.x)))


#for teens
hist(dat.join$Dem_AgeT.x, main="", xlab="Adolescent Age")
ggplot(dat.join, aes(x=Dem_EducationT.x.x)) + geom_bar() + labs(x="Adolescent Education Level")
describe(dat.join[98:107], skew=F)
describe(dat.join$PRE_PSE_enrolled.y)
summary(as.factor(dat.join$Dem_ReadT.x))
28/83 #cannot read
48/83 #can read with much diff
7/83 #can read with little to no diff

#father in HH
summary(as.factor(dat.join$Dem_Father.x.y))[1]/sum(summary(as.factor(dat.join$Dem_Father.x.y)))
#father deceased
summary(as.factor(dat.join$Dem_Father.x.y))[2]/sum(summary(as.factor(dat.join$Dem_Father.x.y)))
#mother in HH
summary(as.factor(dat.join$Dem_Mother.x.y))[1]/sum(summary(as.factor(dat.join$Dem_Mother.x.y)))
#mother deceased
summary(as.factor(dat.join$Dem_Mother.x.y))[2]/sum(summary(as.factor(dat.join$Dem_Mother.x.y)))


#================================ ITEM DISTRIBUTIONS 1 ================================
# STACKED BARPLOTS
library(ggplot2)
library(ggpubr)
...
#================================ BIVARIATE
ggplot(datcg.join, aes(x=Summed_PRE_APQ, y=Summed_PRE_SDQ)) + geom_point()
ggplot(datcg.join, aes(x=Summed_PRE_APQ, y=Summed_PRE_ICAST)) + geom_point()
ggplot(datcg.join, aes(x=Summed_PRE_APQ, y=Summed_PRE_FFC)) + geom_point()

ggplot(datcg.join, aes(x=Summed_PRE_SDQ, y=Summed_PRE_ICAST)) + geom_point()
ggplot(datcg.join, aes(x=Summed_PRE_SDQ, y=Summed_PRE_FFC)) + geom_point()
...
library(ltm)
#TEENS
cronbach.alpha(datteen.join[,13:18], na.rm = T, CI=T) #APQ (PRE)
cronbach.alpha(datteen.join[,46:51], na.rm = T, CI=T) #APQ (POST)
cronbach.alpha(datteen.join[,19:28], na.rm = T, CI=T) #SDQ (PRE)
cronbach.alpha(datteen.join[,52:61], na.rm = T, CI=T) #SDQ (POST)
cronbach.alpha(datteen.join[,29:32], na.rm = T, CI=T) #ICAST (PRE)
cronbach.alpha(datteen.join[,62:65], na.rm = T) #ICAST (POST) [no variation]

#CAREGIVERS
cronbach.alpha(datcg.join[,18:23], na.rm=T, CI=T) #APQ (PRE)
cronbach.alpha(datcg.join[,59:64], na.rm=T, CI=T) #APQ (POST)
cronbach.alpha(datcg.join[,24:28], na.rm=T, CI=T) #SDQ (PRE)
cronbach.alpha(datcg.join[,65:69], na.rm=T) #SDQ (POST)
cronbach.alpha(datcg.join[,29:32], na.rm=T, CI=T) #ICAST (PRE)
cronbach.alpha(datcg.join[,70:73], na.rm=T) #ICAST (POST)
cronbach.alpha(datcg.join[,34:36], na.rm=T) #CESD (PRE)
cronbach.alpha(datcg.join[,75:77], na.rm=T, CI=T) #CESD (POST)
cronbach.alpha(datcg.join[,37:41], na.rm=T, CI=T) #FFC (PRE)
cronbach.alpha(datcg.join[,78:82], na.rm=T, CI=T) #FFC (POST)
...
CFA.PRET.models <- ' APQ_PRE =~ PRE_APQ_talk + PRE_APQ_involved + PRE_APQ_talk_friends + PRE_APQ_evening + PRE_APQ_time + PRE_APQ_dark
                SDQ_PRE =~ PRE_SDQ_tantrums + PRE_SDQ_obedient + PRE_SDQ_fights + PRE_SDQ_lies + PRE_SDQ_steals + PRE_SDQ_worried + PRE_SDQ_unhappy + PRE_SDQ_new + PRE_SDQ_fears
                ICAST_PRE =~ PRE_ICAST_spank+ PRE_ICAST_scream + PRE_ICAST_upset + PRE_ICAST_object  '
cfa.t.pre <- cfa(CFA.PRET.models, data = datteen.join)
summary(cfa.t.pre, fit.measures=T, standardized=T) #SDQ and ICAST have good internal consistency

#ICAST and SDQ items have too little variation
...
summary(cfa.t.post, fit.measures=T, standardized=T)
